...
mongoose.connect("mongodb+srv://server:Password@cluster0.io7bh.mongodb.net/LippearShop?retryWrites=true&w=majority&appName=Cluster0")
	.then(async () => {
...
app.get("/api/fragrances", async (req, res) => {
    ...
    const fragranceLimitOnPage = 6;
		const searchWords = search ? search.trim().split(/\s+/).map(word => new RegExp(word, "i")) : [];
		const countToSkip = (page-1)*fragranceLimitOnPage;
		const DBfilter = {$and:[]};
    console.log(searchWords)

		if(brand) DBfilter.$and.push({brand: brand});
		...
		const fragrances = await Fragrance.find(DBfilter).skip(countToSkip).limit(fragranceLimitOnPage);
...
app.get("/api/searchQuery", async (req, res) => {
	try {
		...
		const searchResults = results.map(result => `${result.brand} ${result.name}`);
