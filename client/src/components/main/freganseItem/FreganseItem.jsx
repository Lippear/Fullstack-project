...
import {
  ...
  addItemToCart,
  openCart
} from '../../../redux-toolkit/cart/cartSlise'
import './FreganseItem.scss'

export default function FreganseItem({ perfume }) {
  const [choosenPerfumeIndex, setChoosenPerfumeIndex] = useState(
    perfume.mainVolumeIndex
  )
  ...
  const isItemInCart = useSelector((state) =>
    selectIsItemInCart(state, perfume.id, choosenPerfumeIndex)
  )
  ...
  return (
    <div className="freganse__item">
      <div className="freganse__info">
        <img
          src={perfume.photo}
          className="freganse__item__image"
          alt={perfume.name}
        />
        <span className="brand">{perfume.brand}</span>
        <span className="name">{perfume.name}</span>
        <div
          className="volume__changer"
          ref={refVolumeChanger}
          onMouseLeave={() => setIsOpenChooseSection(false)}
        >
          <Button
            className="volume__btn items__btn"
            onClick={() => setIsOpenChooseSection(!isOpenChooseSection)}
          >
            <span className="volume__info">
              {perfume.volumesAndPrices[choosenPerfumeIndex].volume} {t('ml')}
            </span>
            {perfume.volumesAndPrices.length > 1 && (
              <span
                className={`choose__icon arrow ${isOpenChooseSection ? 'rotate' : ''}`}
              >
                ▼
              </span>
            )}
